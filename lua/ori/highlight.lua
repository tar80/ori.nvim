...
function M.combine(mode)
  local function _combine_light(target, base, intensity)
    intensity = intensity or 1
    return math.max(0, math.floor(target[1] - (255 - base[1]) * intensity)),
      math.max(0, math.floor(target[2] - (255 - base[2]) * intensity)),
      math.max(0, math.floor(target[3] - (255 - base[3]) * intensity))
  end

  local function _combine_dark(target, base, intensity)
    intensity = intensity or 1
    local sr = 255 - ((255 - target[1]) * (255 - base[1]) / 255)
    local sg = 255 - ((255 - target[2]) * (255 - base[2]) / 255)
    local sb = 255 - ((255 - target[3]) * (255 - base[3]) / 255)

    return math.floor(target[1] + (sr - target[1]) * intensity),
      math.floor(target[2] + (sg - target[2]) * intensity),
      math.floor(target[3] + (sb - target[3]) * intensity)
  end
